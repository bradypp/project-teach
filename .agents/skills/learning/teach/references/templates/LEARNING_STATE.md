# Learning state

A concise current view of understanding and worthwhile possibilities. Link to supporting records instead of repeating their evidence. Omit empty sections.

## Current understanding

- **{{Concept}}:** {{Narrow supported understanding and important limits.}} [Record]({{record path}})

## Uncertainty and next practice

- **{{Concept}}:** {{What is unclear or worth checking in context.}}

## Opportunities

- **{{Concept}}:** {{What prompted discovery and why it might matter, even beyond this project.}} {{Useful link if available.}}

## Useful lessons

- [{{Lesson title}}]({{lesson path}}) — {{When to revisit it.}}
