...
(async () => {
  ...
  try {
    ...
    assert.equal(chatCopy.split("\n")[0], `Please review my responses to [Practice & feedback](${file}). Give me feedback on my reasoning, then ask 1-3 useful follow-up questions.`);
    assert.ok(!chatCopy.includes("Local file:"));
