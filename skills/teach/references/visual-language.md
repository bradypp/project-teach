...
- **Diagrams:** Start from the [diagram template](../assets/templates/diagram.html) and follow [Mermaid guidance](mermaid.md). Use compact horizontal flows when they fit, and vertical or split flows when they become crowded. Wrap labels where useful; the renderer shrinks to fit without horizontal scrolling. Keep horizontal paths to four or five stages; use vertical or split layouts beyond that. Follow the Mermaid guidance for composition and readability checks. Keep themes and palette directives out of individual diagrams.
